...
FilePath.add = function (data) {
    const {type,path,patientId,patientName} = data;
    return new Promise((resolve,reject)=>{
        Model.findOne({path},'path',(err,model)=>{
            //console.log("@@@@@:",err,"222222222:",model);
            if (err) throw err;
            if(model){
                console.log('新增,item已存在:'+path);
                reject('新增,item已存在11:'+path)
            }else{
                var data = new Model({
                    type,path,patientId,patientName
                }).save(function (error) {
                    if(error){
